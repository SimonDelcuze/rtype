...
struct RollbackRequestPacket
{
    PacketHeader header{};
    std::uint32_t playerId       = 0;
    std::uint64_t rollbackToTick = 0;
    std::uint64_t currentTick    = 0;
    std::uint8_t reason          = 0;

    static constexpr std::size_t kPayloadSize = 4 + 8 + 8 + 1;
    ...
    [[nodiscard]] std::array<std::uint8_t, kSize> encode() const noexcept
    {
        ...
        h.messageType  = static_cast<std::uint8_t>(MessageType::RollbackRequest);
    ...
    [[nodiscard]] static std::optional<RollbackRequestPacket> decode(const std::uint8_t* data, std::size_t len) noexcept
    {
        ...
        if (!hdr)
            return std::nullopt;
        if (hdr->messageType != static_cast<std::uint8_t>(MessageType::RollbackRequest))
            return std::nullopt;
        ...
        auto r64 = [&](std::uint64_t& v) {
            v = (static_cast<std::uint64_t>(data[o]) << 56) | (static_cast<std::uint64_t>(data[o + 1]) << 48) |
                (static_cast<std::uint64_t>(data[o + 2]) << 40) | (static_cast<std::uint64_t>(data[o + 3]) << 32) |
                (static_cast<std::uint64_t>(data[o + 4]) << 24) | (static_cast<std::uint64_t>(data[o + 5]) << 16) |
                (static_cast<std::uint64_t>(data[o + 6]) << 8) | static_cast<std::uint64_t>(data[o + 7]);
            o += 8;
        };

        RollbackRequestPacket p{};
        p.header = *hdr;
        r32(p.playerId);
        r64(p.rollbackToTick);
        r64(p.currentTick);
        p.reason = data[o++];
...
static_assert(RollbackRequestPacket::kSize == 42, "RollbackRequestPacket wire size must remain 42 bytes");
