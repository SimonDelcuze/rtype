...
struct StateChecksumPacket
{
    ...
    static constexpr std::size_t kPayloadSize = 4 + 8 + 4 + 4;
    ...
    [[nodiscard]] std::array<std::uint8_t, kSize> encode() const noexcept
    {
        ...
        h.messageType  = static_cast<std::uint8_t>(MessageType::StateChecksum);
    ...
    [[nodiscard]] static std::optional<StateChecksumPacket> decode(const std::uint8_t* data, std::size_t len) noexcept
    {
        ...
        if (!hdr)
            return std::nullopt;
        if (hdr->messageType != static_cast<std::uint8_t>(MessageType::StateChecksum))
            return std::nullopt;
...
static_assert(StateChecksumPacket::kSize == 41, "StateChecksumPacket wire size must remain 41 bytes");
