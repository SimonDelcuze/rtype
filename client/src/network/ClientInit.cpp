...
{
    UdpSocket sock;
    if (!sock.open(IpEndpoint::v4(0, 0, 0, 0, 0))) {
        std::cerr << "Failed to open hello socket\n";
        return;
    }
...
void sendJoinRequestOnce(const IpEndpoint& server, std::uint16_t sequence)
{
    UdpSocket sock;
    if (!sock.open(IpEndpoint::v4(0, 0, 0, 0, 0))) {
        std::cerr << "Failed to open join socket\n";
        return;
    }
...
void sendClientReadyOnce(const IpEndpoint& server, std::uint16_t sequence)
{
    UdpSocket sock;
    if (!sock.open(IpEndpoint::v4(0, 0, 0, 0, 0))) {
        std::cerr << "Failed to open ready socket\n";
        return;
    }
...
void sendPingOnce(const IpEndpoint& server, std::uint16_t sequence)
{
    UdpSocket sock;
    if (!sock.open(IpEndpoint::v4(0, 0, 0, 0, 0))) {
        std::cerr << "Failed to open ping socket\n";
        return;
    }
...
void sendWelcomeLoop(const IpEndpoint& server, std::atomic<bool>& stopFlag)
{
    std::uint16_t seq = 0;
    while (!stopFlag.load()) {
        sendClientHelloOnce(server);
        sendJoinRequestOnce(server, ++seq);
        sendClientReadyOnce(server, ++seq);
        sendPingOnce(server, ++seq);
