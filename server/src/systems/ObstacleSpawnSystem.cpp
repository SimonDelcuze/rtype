...
void ObstacleSpawnSystem::spawnOne(Registry& registry, const ObstacleVariant& variant)
{
    EntityId e      = registry.createEntity();
    auto& transform = registry.emplace<TransformComponent>(e);
    transform.x     = config_.spawnX;
    transform.y     = std::max(0.0F, WorldConfig::Height - variant.spriteHeight);
    ...
    registry.emplace<TypeComponent>(e, TypeComponent::create(variant.typeId));

    float marginX = std::max(0.0F, (variant.spriteWidth - variant.hitboxWidth) / 2.0F);
    float marginY = std::max(0.0F, (variant.spriteHeight - variant.hitboxHeight) / 2.0F);
    float offsetX = marginX + variant.hitboxWidth / 2.0F;
    float offsetY = marginY + variant.hitboxHeight / 2.0F;
