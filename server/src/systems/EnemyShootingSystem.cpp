...
void EnemyShootingSystem::update(Registry& registry, float deltaTime)
{
    ...
    for (EntityId id : enemies) {
        ...
        if (shooting.timeSinceLastShot >= shooting.shootInterval) {
            ...
            registry.emplace<MissileComponent>(projectile, 
                MissileComponent{shooting.projectileDamage, shooting.projectileLifetime, false});
            ...
            registry.emplace<HitboxComponent>(projectile, 
                HitboxComponent::create(20.0F, 20.0F, 0.0F, 0.0F, true));
        }
    }
}
