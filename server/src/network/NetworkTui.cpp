...
NetworkTui::NetworkTui() : _lastUpdate(std::chrono::steady_clock::now()), _startTime(std::chrono::steady_clock::now()) {
    ...
    for (int i = 0; i < _width; ++i) _bandwidthHistory.push_back(0.0f);
}
...
void NetworkTui::update(const NetworkStats& stats) {
    ...
    if (dt >= 0.1) {
        ...
        for (float val : _bandwidthHistory) {
            if (val > _maxBandwidth) _maxBandwidth = val;
        }
...
void NetworkTui::drawHeader(std::ostringstream& frame) {
    ...
    if (space > 0) frame << repeatString(" ", space);
...
void NetworkTui::drawStats(std::ostringstream& frame) {
    frame << pos(3, 1) << "  "
          << BOLD << FG_GREEN << "IN: " << RESET << std::setw(10) << std::left << formatBytes(_currentStats.bytesIn) 
          << BOLD << FG_CYAN << " PACKETS: " << RESET << std::setw(8) << std::left << _currentStats.packetsIn
          << " | "
          << BOLD << FG_BLUE << "OUT: " << RESET << std::setw(10) << std::left << formatBytes(_currentStats.bytesOut)
          << BOLD << FG_CYAN << " PACKETS: " << RESET << std::setw(8) << std::left << _currentStats.packetsOut
          << " | "
          << BOLD << FG_RED << "LOSS: " << RESET << _currentStats.packetsLost
          << clr() << "\n";
    frame << pos(4, 1) << clr() << "\n";
}
...
void NetworkTui::drawGraph(std::ostringstream& frame) {
    ...
    for (int h = graphHeight; h > 0; --h) {
        ...
        if (h == graphHeight) {
            frame << std::setw(10) << std::right << formatBytes((size_t)_maxBandwidth) << "/s" << DIM << " ┐ " << RESET;
        } else if (h == 1) {
            frame << std::setw(10) << std::right << "0 B/s" << DIM << " ┘ " << RESET;
        } else if (h == (graphHeight / 2) + 1) {
            frame << std::setw(10) << std::right << formatBytes((size_t)(_maxBandwidth / 2)) << "/s" << DIM << " ┤ " << RESET;
        } else {
        ...
        for (int i = 0; i < graphWidth; ++i) {
            size_t historyIdx = 0;
            if (historySize > (size_t)graphWidth) historyIdx = historySize - graphWidth + i;
            else if (i < (graphWidth - (int)historySize)) { frame << " "; continue; }
            else historyIdx = i - (graphWidth - historySize);
            
            float val = _bandwidthHistory[historyIdx] / _maxBandwidth;
            int currentLevel = (int)(val * graphHeight * 8) - (h - 1) * 8;
            
            if (currentLevel <= 0) frame << " ";
            else if (currentLevel >= 8) frame << FG_CYAN << blocks[8] << RESET;
            else frame << FG_CYAN << blocks[currentLevel] << RESET;
        }
        frame << clr() << "\n";
    }
    frame << pos(startLine + graphHeight + 1, 1) << "  " << DIM << "└" << repeatString("─", graphWidth) << RESET << clr() << "\n";
}
...
void NetworkTui::drawLogs(std::ostringstream& frame) {
    ...
    for (size_t i = 0; i < (size_t)logLines; ++i) {
        ...
        if (startIdx + i < _logs.size()) {
            ...
            if (line.size() > (size_t)_width - 4) line = line.substr(0, _width - 7) + "...";
            if (!line.empty() && line.back() == '\n') line.pop_back();
            std::string color = RESET;
            if (line.find("[ERROR]") != std::string::npos) color = FG_RED;
            else if (line.find("[WARN]") != std::string::npos) color = FG_YELLOW;
