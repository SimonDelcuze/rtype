...
void GameInstance::captureStateSnapshot()
{
    ...
    {
        Logger::instance().info("[Rollback] Captured state snapshot at tick " +
                                std::to_string(currentTick_) + " checksum=0x" +
...
{
    Logger::instance().warn("[Rollback] Desynchronization detected for player " +
                            std::to_string(desyncInfo.playerId) + " at tick " +
                            std::to_string(desyncInfo.tick) + ": " + desyncInfo.description);
    ...
    if (!found)
    {
        Logger::instance().warn("[Rollback] Could not find client for player " +
                                std::to_string(desyncInfo.playerId));
        return;
    }
    ...
    Logger::instance().info("[Rollback] Requesting client rollback to tick " +
                            std::to_string(rollbackToTick) + " (current: " +
                            std::to_string(currentTick_) + ")");
