        ...
        while (receiveRunning_) {
...

            {
                ...
                for (auto it = lobbySessions_.begin(); it != lobbySessions_.end();) {
                    auto elapsed =
                        std::chrono::duration_cast<std::chrono::seconds>(now - it->second.lastActivity).count();
                    if (elapsed > 30) {
                                       ...
                                       const IpEndpoint& from)
{
        ...
        bool isExpiredRanked = (roomInfo->roomType == RoomType::Ranked && players.size() >= 2 &&
                                lobbyManager_.getRoomCountdown(roomId) == 0);
        ...
        Logger::instance().info("[LobbyServer] Broadcast complete: sent to " + std::to_string(broadcastCount) +
                                " players");
}
            ...
            for (const auto& [sessionKey, session] : lobbySessions_) {
                if (session.authenticated && session.userId.has_value() && session.userId.value() == user->id &&
                    ...
                    Logger::instance().warn("[LobbyServer] Login failed: account already connected - " +
                                            loginData->username);
                    auto response =
                        buildLoginResponsePacket(false, 0, "", AuthErrorCode::AlreadyConnected, hdr.sequenceId);
                    sendPacket(response, from);
                    return;
                }
            }
                                        ...
                                        const IpEndpoint& from)
{
        ...
        if (existingUser.has_value()) {
            Logger::instance().warn("[LobbyServer] Register failed: username already taken - " +
...
void LobbyServer::handleGetStatsRequest(const PacketHeader& hdr, const IpEndpoint& from)
{
        ...
        Logger::instance().info("[LobbyServer] Sending stats for user " + session.username + ": games=" +
                                std::to_string(stats->gamesPlayed) + ", wins=" + std::to_string(stats->wins));
